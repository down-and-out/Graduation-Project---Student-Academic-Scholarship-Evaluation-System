-- ========================================
-- Flyway 数据库版本管理 - V1.3 修复字段不匹配问题（精确版）
-- ========================================
-- 根据实际数据库结构，只添加缺失的字段
-- ========================================
...
ADD COLUMN `authorization_date_new` DATE DEFAULT NULL COMMENT '授权日期' AFTER `application_date`,
...
-- 数据迁移：将 grant_date 复制到 authorization_date_new
UPDATE `research_patent` SET `authorization_date_new` = `grant_date` WHERE `authorization_date_new` IS NULL AND `grant_date` IS NOT NULL;
