...
CREATE TABLE `evaluation_batch` (
    ...
    `batch_year` INT NOT NULL COMMENT '评定年份',
    `batch_type` TINYINT NOT NULL COMMENT '评定类型：1-春季奖学金 2-秋季奖学金 3-专项奖学金',
    `start_date` DATE NOT NULL COMMENT '申请开始日期',
    `end_date` DATE NOT NULL COMMENT '申请结束日期',
    `review_start_date` DATE NOT NULL COMMENT '评审开始日期',
    `review_end_date` DATE NOT NULL COMMENT '评审结束日期',
    `total_quota` INT DEFAULT NULL COMMENT '总名额',
    `scholarship_amount` DECIMAL(10,2) DEFAULT NULL COMMENT '奖学金总额（万元）',
    `description` VARCHAR(500) DEFAULT NULL COMMENT '批次说明',
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '状态：0-未开始 1-进行中 2-评审中 3-已完成 4-已取消',
    `deleted` TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0-未删除 1-已删除',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
