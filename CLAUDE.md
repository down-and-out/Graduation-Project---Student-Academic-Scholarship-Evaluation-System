...
**每次修改代码文件（`.java`、`.ts`、`.vue`、`.xml`、`.sql`）时，必须为新写或修改的代码添加中文注释。** 注释遵循以下规则：

### 必须注释
1. **类/接口/组件**：文件顶部一句话描述职责
2. **公共方法/函数**：JSDoc/Javadoc 说明用途、参数、返回值
3. **复杂业务逻辑**：分段注释说明步骤（如评定计算、状态流转）
4. **非显而易见的判断**：解释"为什么这么做"（边界条件、特殊处理）
5. **魔法数字/常量**：解释含义和来源
6. **API 调用**：说明目的和关键参数
7. **Vue 模板关键区块**：`<!-- -->` 说明条件渲染/列表过滤逻辑

### 不需要注释
- 简单的 getter/setter
- 标准 CRUD（如 `mapper.selectById(id)`）
- 自解释的变量名/函数名
- 框架标准写法（如 MyBatis-Plus `LambdaQueryWrapper`）

### 风格要点
- 注释说"为什么"而非"做什么"，保持简洁
- Java 后端：Javadoc 格式（`/** ... @param ... @return */`）+ `//` 行注释
- Vue/TS：JSDoc 格式 + `//` 行注释，模板用 `<!-- -->`
- SQL/XML Mapper：`<!-- -->` 说明查询目的
- 中文简体，术语可保留英文
